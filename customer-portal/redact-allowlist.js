/**
 * Browser copy of functions/lib/redactAllowlist.js — tests/validation only.
 * Snapshots are created server-side; this file must stay in sync with the Functions module.
 */
(function(){
  "use strict";
  ...
  function redactPublicSnapshot(customer,options){
    ...
    if(window.ACTBookingLibrary){
      const applied=window.ACTBookingLibrary.applyBookingsToProgram(next);
      next.program=applied.program;
      next.programItems=applied.program;
      next.bookings=window.ACTBookingLibrary.publishedBookings(next);
    }else{
